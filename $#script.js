...
function searchMembers() {
    ...
    // Variabel day, month, year dihapus
    ...
    const results = allMembers.filter(member => {
        ...
        // Logika matchesDate dihapus
        return matchesSearchTerm; // Hanya kembalikan berdasarkan search term
    });
    displaySearchResults(results.reverse(), allMembers);
}
// === AKHIR PERUBAHAN ===

// Fungsi ini tidak diubah, karena sudah membuat kartu vertikal
...
function resetSearch() {
    ...
    // Baris untuk searchDay, searchMonth, searchYear dihapus
...
// ==========================================================
// === FUNGSI INI DIPERBARUI (renderNetwork) ===
// ==========================================================
function renderNetwork() {
    ...
    const allMembers = loadMembers(); // Selalu muat semua anggota
    ...
    let membersToRender; // Anggota yang akan ditampilkan di diagram

    if (focusedMemberUid) {
        // --- LOGIKA BARU: Tampilkan hierarki (akar pohon) ---
        const rootMember = allMembers.find(m => m.uid === focusedMemberUid);
        
        if (rootMember) {
            // Fungsi helper untuk mencari semua bawahan (downline) secara rekursif
            const getDownlineHierarchy = (allMembersList, parentUid) => {
                let downlines = [];
                const directChildren = allMembersList.filter(m => m.upline === parentUid);
                for (const child of directChildren) {
                    downlines.push(child); // Tambahkan anak langsung
                    // Cari bawahan dari anak ini (cucu, cicit, dst.)
                    const childDownlines = getDownlineHierarchy(allMembersList, child.uid);
                    downlines = downlines.concat(childDownlines); // Gabungkan hasilnya
                }
                return downlines;
            };
            
            // Mulai render dari anggota yang difokuskan, dan tambahkan semua bawahannya
            membersToRender = [rootMember, ...getDownlineHierarchy(allMembers, focusedMemberUid)];
        } else {
            // Fallback jika UID aneh, tampilkan semua
            membersToRender = allMembers;
        }
        // --- AKHIR LOGIKA BARU ---
    } else {
        // --- LOGIKA LAMA: Tampilkan semua jaringan jika tidak ada fokus ---
        membersToRender = allMembers;
    }
    
    // Hitung jumlah downline berdasarkan SEMUA anggota, bukan hanya yg dirender
    ...
    // Buat Nodes dan Links HANYA dari 'membersToRender'
    const nodes = membersToRender.map(m => ({ key: m.uid, label: `${m.name}\n(UID: ${m.uid})` }));
    const links = membersToRender
        .filter(m => m.upline && membersToRender.some(u => u.uid === m.upline)) // Pastikan upline-nya juga ada di daftar render
        .map(m => ({ from: m.upline, to: m.uid }));
        
    diagram.model = new go.GraphLinksModel(nodes, links);

    // Logika fokus (tetap penting, untuk memusatkan)
    if (focusedMemberUid) {
        ...
        sessionStorage.removeItem('focusedMemberUid'); // Hapus setelah digunakan
    }
}
// ==========================================================
// === AKHIR PERUBAHAN ===
