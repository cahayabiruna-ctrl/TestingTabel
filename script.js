...
// Variabel baru untuk status sorting tabel
...
// (Fungsi Notifikasi, Modal, CRUD, CSV, dan Search tetap sama persis seperti sebelumnya)
// ...
// --- NOTIFICATION & MODAL ---
...
/**
 * Menampilkan kembali konten dashboard utama dan menyembunyikan daftar anggota.
 */
...
/**
 * Menampilkan daftar anggota dan menyembunyikan konten dashboard utama.
 */
function showMemberList() {
    ...
    // Render tabel dengan data dan sort default
    renderMemberList(); 
}

/**
 * Menambahkan event listener ke header tabel untuk sorting.
 */
...
/**
 * Merender (menggambar) data anggota ke dalam tabel, lengkap dengan logika sorting.
 */
function renderMemberList() {
    ...
    if (memberListSortColumn === 'no') {
        // Sortir berdasarkan 'No.' sama dengan sortir kronologis (Tgl Bergabung)
        sortedMembers = members.sort((a, b) => new Date(a.joinDate) - new Date(b.joinDate));
        if (memberListSortDirection === 'desc') {
            sortedMembers.reverse(); // Balik urutan jika 'desc'
        }
    } else {
        // Sortir berdasarkan kolom lain
        sortedMembers = members.sort((a, b) => {
            ...
            switch (memberListSortColumn) {
                ...
                case 'uid':
                    ...
                    break;
                case 'upline':
                    valA = (a.upline || '').toLowerCase(); // Tangani jika upline null
    ...
    // Update Indikator Visual (Panah) di Header Tabel
    ...
    // Isi Tabel dengan Data yang Sudah Disortir
    sortedMembers.forEach((member, index) => {
        const joinDate = member.joinDate ? new Date(member.joinDate).toLocaleDateString('id-ID') : 'N/A';
        const row = `
            <tr>
                <td>${index + 1}</td> <td>${member.name}</td>
                <td>${member.uid}</td>
                <td>${member.upline || '-'}</td>
                <td>${joinDate}</td>
            </tr>
